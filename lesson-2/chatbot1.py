...
class DB:
    ...
    def load_history(self, user_id):
        self.cur.execute(
            "select message, role from history where user_id=? order by user_message_id;",
            (user_id,)
        )
    ...
    def save_message(self, user_id, message, role):
        self.cur.execute(
            "select max(user_message_id) from history where user_id=?;",
            (user_id,)
        )
        row = self.cur.fetchone()
        ...
        self.cur.execute(
            "insert into history (user_id, user_message_id, message, role) values (?, ?, ?, ?);",
...
"""
    ...
    def register(self, username=None):
        ...
        if password1 != password2:
            ...
            return self.register(username=username)
